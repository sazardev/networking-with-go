...
func listWirelessCandidates() ([]net.Interface, error) {
	...
	for _, iface := range ifaces {
		// A naming convention heuristic (wlan0, wlp2s0, en0 on macOS Wi-Fi, etc.).
		// It's not authoritative—use OS tools below for a definitive answer.
...
func scanWifi() ([]WifiNetwork, error) {
	...
	cmd := exec.CommandContext(ctx, "nmcli", "-t", "-f", "SSID,SIGNAL,CHAN", "dev", "wifi", "list")
	out, err := cmd.Output()
	if err != nil {
	...
	for scanner.Scan() {
		...
		networks = append(networks, WifiNetwork{SSID: fields[0], Signal: signal, Channel: fields[2]})
	}
...
result, err := params.Get([]string{".1.3.6.1.4.1.9.9.618.1.8.1.0"}) // vendor-specific OID
```

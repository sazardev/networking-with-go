...
<Warning title="This is a genuine ethical and legal boundary, not a footnote">
Inspecting traffic you do not own or are not authorized to monitor is illegal in most jurisdictions and is a serious breach of trust even where it is technically legal (for example, inspecting traffic on your own home lab or with explicit, informed consent from all parties). Everything in this chapter assumes you are working with traffic you have clear authority to capture -- your own test network, your own application's traffic, or an environment where you have documented authorization.
</Warning>
...
<DeepDive title="Root privileges and platform setup">
Raw packet capture requires elevated privileges on every platform -- `CAP_NET_RAW` (or root) on Linux, Administrator on Windows with Npcap installed, and root on macOS -- because reading traffic that was not addressed to your process is precisely the capability an operating system restricts by default. `gopacket/pcap` is a cgo binding to `libpcap`/`Npcap`, so those native libraries and headers need to be installed on the build machine as well as the target machine.
</DeepDive>

...
func (rt *RoutingTable) Update(dest, viaNeighbor string, advertisedCost int) bool {
	...
	// Adopt the route if it's better, new, or a refresh from our current next hop
	// (so that a neighbor's own cost increases correctly propagate to us too).
	if !exists || newCost < current.Cost || current.NextHop == viaNeighbor {
		if exists && current.NextHop == viaNeighbor && newCost == current.Cost {
			rt.routes[dest] = Route{NextHop: viaNeighbor, Cost: newCost, Updated: time.Now()}
...
func advertiseLoop(nodeName string, rt *RoutingTable, neighbors []string, interval time.Duration) {
	...
	for range ticker.C {
		snap := rt.Snapshot()
		ad := Advertisement{From: nodeName, Routes: map[string]int{nodeName: 0}}
...
<Warning title="Loop prevention needs more than this sketch">
This simplified example omits split-horizon and hop-limit handling, both of which real distance-vector protocols need to avoid routing loops and "count-to-infinity" problems when a route disappears. Production implementations (like RIP's split-horizon-with-poison-reverse) add these explicitly; don't deploy the sketch above unmodified.
</Warning>

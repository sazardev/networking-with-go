...
<Warning title="Raw 802.11 injection is not portable">Frame injection and monitor-mode capture are possible on Linux with driver support, root privileges, and either cgo bindings to `libnl`/`libpcap` or shelling out to tools like `iw`. There is no cross-platform Go API for this, and production code should not depend on it.</Warning>

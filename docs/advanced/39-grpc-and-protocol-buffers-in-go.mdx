...
<DeepDive title="Why the field numbers matter forever">
Once a message ships to production, its field numbers are a contract. Renaming a Go struct field is free; renumbering a protobuf field is a breaking change, because the receiver decodes by tag number, not by name. Removing a field safely means marking its number `reserved`, never reusing it for something else.
</DeepDive>
...
<Warning title="insecure.NewCredentials() is for development only">

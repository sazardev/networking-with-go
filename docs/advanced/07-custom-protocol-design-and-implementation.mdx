...
<DeepDive title="State machines keep protocols honest">Beyond framing, most real protocols have a notion of valid message *order* -- a handshake must complete before data messages are accepted, a `Close` message ends the conversation. Modeling this explicitly as a state machine (a `type connState int` with defined transitions, rejecting messages that arrive in the wrong state) turns "what should never happen" into something your code actively checks for, instead of an assumption baked silently into control flow.</DeepDive>
...
**Why not just use delimiters like HTTP/1.1 does, instead of a length-prefixed header?**

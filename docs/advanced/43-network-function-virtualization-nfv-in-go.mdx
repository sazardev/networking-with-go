...
func newLoadBalancerVNF(backendURLs []string) *loadBalancerVNF {
	...
	for _, raw := range backendURLs {
		...
		lb.backends = append(lb.backends, httputil.NewSingleHostReverseProxy(target))
	}
	return lb
}
...
<Warning title="This example manages containers, not the network path itself">

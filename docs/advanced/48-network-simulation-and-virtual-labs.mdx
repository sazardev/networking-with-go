...
<DeepDive title="Why not just call Docker's Go SDK directly?">`github.com/docker/docker/client` is a real, officially maintained option, and production tooling (Containerlab included) does use it. This chapter favors `os/exec` against the `docker` CLI instead because the CLI's behavior is stable and easy to verify by hand, whereas the client SDK's API shifts across Docker Engine versions -- for a teaching example, a shorter, verifiably correct approach beats a longer one that risks drifting out of sync with a specific SDK version.</DeepDive>

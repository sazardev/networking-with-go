...
<DeepDive title="NETCONF and gNMI, briefly">
NETCONF (RFC 6241) runs over SSH and exchanges XML "configuration datastores." gNMI is Google's newer, gRPC-based equivalent, exchanging protobuf messages and supporting streaming telemetry as well as configuration. Both are modeled against YANG schemas defined by the OpenConfig project or the vendor. Go has real client libraries for both ecosystems (for example `openconfig/gnmic` for gNMI), but the concepts—not a specific API—are what you need to recognize when you meet them in a real network.
</DeepDive>
...
func runCommand(addr, user, password, cmd string) (string, error) {
	config := &ssh.ClientConfig{
		...
		HostKeyCallback: ssh.InsecureIgnoreHostKey(), // demo only: verify real host keys in production
...
<Warning title="Never ship InsecureIgnoreHostKey">
`ssh.InsecureIgnoreHostKey()` accepts any host key, which means it cannot detect a man-in-the-middle attack. Real automation tooling verifies the host key against a known-hosts file with `ssh.FixedHostKey` or a custom `HostKeyCallback`.
</Warning>

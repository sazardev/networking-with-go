...
have all eight chapters of this part in your hands: variables that never
lie about their type, functions that report failure honestly, structs and
interfaces instead of classes, errors instead of exceptions, packages and
modules for real projects, tests built into the toolchain, and now,
goroutines and channels for doing many things at once. Part 2 picks up
exactly where this chapter left off, and the very first thing it does is
open a real network socket — at which point every one of these eight
chapters stops being an exercise and starts being the thing you actually
reach for, on every single page, for the rest of this book.

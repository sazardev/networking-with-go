...
func triggerDeployWorkflow(ctx context.Context, client *github.Client, owner, repo string) error {
...
func rollbackHandler(w http.ResponseWriter, r *http.Request) {
	...
	logger.Warn("rolled back", "service", service, "version", previous, "actor", currentUser(r))
	writeJSON(w, http.StatusOK, map[string]string{"status": "rolled back", "version": previous})
}
```

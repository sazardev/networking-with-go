	...
	for w := 0; w < concurrency; w++ {
		...
		go func() {
			...
			for range jobs {
				...
				if err != nil {
					results <- result{duration: elapsed, err: err}
					continue
				}
				resp.Body.Close()
...
<Warning title="A custom load generator on the same machine as your test client shares its network stack">
If your Go load generator and the service under test run on the same box, you're measuring loopback performance, not real network behavior, and you can exhaust the client machine's own ephemeral port range or CPU before the server even notices load. For anything beyond quick local sanity checks, run the generator from a separate machine (or several).
</Warning>
...
func TestPerformanceRegression(t *testing.T) {
	...
	if metrics.Latencies.P95 > 200*time.Millisecond {
		t.Fatalf("p95 latency regressed: got %s, budget 200ms", metrics.Latencies.P95)
	}
	if metrics.Success < 0.99 {
		t.Fatalf("success ratio regressed: got %.2f%%, budget 99%%", metrics.Success*100)
	}
}
```

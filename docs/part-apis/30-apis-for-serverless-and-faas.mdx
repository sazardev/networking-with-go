...
<Warning title="A database connection per invocation doesn't behave like a connection pool">If your Lambda function opens a new database connection on every cold start and thousands of concurrent invocations spin up under load, you can exhaust your database's connection limit almost immediately — a failure mode that simply doesn't exist with a single long-running server holding one pool. Use a proxy (like AWS RDS Proxy) or a connection-light data store, and keep connection setup in package-level initialization so warm invocations reuse it.</Warning>
...
<DeepDive title="Provisioned concurrency trades cost for cold-start elimination">AWS Lambda's provisioned concurrency keeps a set number of execution environments pre-initialized and warm, effectively buying back the cold-start latency at the cost of paying for that idle capacity — a middle ground between a fully "pay only when invoked" function and a permanently running server. It's worth reaching for on latency-sensitive endpoints once cold starts show up as a real problem in your metrics, not as a default.</DeepDive>

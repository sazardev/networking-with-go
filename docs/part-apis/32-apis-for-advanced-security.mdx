...
func handler(w http.ResponseWriter, r *http.Request) {
	...
	// clientCN now identifies which service is calling, cryptographically verified
}
```
...
```go
func loadSigningKey(ctx context.Context, client *secretsmanager.Client, secretID string) (*rsa.PrivateKey, error) {
...
<DeepDive title="Environment variables are a transport, not a vault">
Environment variables are a reasonable way to *deliver* a secret into a running process, but they aren't a substitute for a system that controls who can read the secret, rotates it, and logs access to it. A secrets manager gives you all three; a `.env` file typically gives you none of them, and it's far too easy for it to end up in a container image, a log line, or a support ticket screenshot.
</DeepDive>
...
func revokeToken(ctx context.Context, rdb *redis.Client, tokenID string, ttl time.Duration) error {

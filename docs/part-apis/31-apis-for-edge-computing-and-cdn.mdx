...
<DeepDive title="Cache keys need to match your actual variance, not less and not more">
If your API returns different content per user (personalized recommendations) but caches on the URL alone, you'll serve one user's personalized data to another. If it returns identical content for everyone but includes `Vary: Cookie` unnecessarily, you fragment the cache into one entry per session and get almost no cache hit rate at all. Get the `Vary` header (or the CDN's equivalent cache-key configuration) to match exactly what actually varies in the response.
</DeepDive>

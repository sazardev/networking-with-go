...
func createOrderHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, "could not queue email", http.StatusInternalServerError)
		return
	}
...
<DeepDive title="Idempotency matters more with queues than without them">
Because a task can be retried, `handleEmailDelivery` might run twice for the same order if the first attempt succeeded but the worker crashed before acknowledging it to Redis. Design task handlers so running them twice is safe — check whether the email was already marked sent before sending it again, the same way you'd design a webhook receiver.
</DeepDive>
...
```go
_, err := asynqClient.Enqueue(task, asynq.ProcessIn(24*time.Hour)) // reminder email tomorrow
```
...
<Warning title="Don't queue what you haven't validated">
Enqueue only after you've validated the request and are confident the work should happen — a queue with no schema validation on the way in becomes a queue full of garbage payloads that workers can't process, discovered only when they start failing at 2 a.m.
</Warning>
...
func queueHealthHandler(inspector *asynq.Inspector) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			http.Error(w, "could not inspect queue", http.StatusInternalServerError)
			return
		}
...
func retryArchivedHandler(inspector *asynq.Inspector) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := inspector.RunTask("default", taskID); err != nil {
			http.Error(w, "could not requeue task", http.StatusInternalServerError)
			return
		}

...
<Warning title="One writer at a time per connection">
`gorilla/websocket` explicitly documents that a connection supports only one concurrent reader and one concurrent writer. If multiple goroutines might write to the same connection — common once you add broadcasting — you must serialize writes yourself, typically with a mutex or by funneling all writes through a single goroutine reading from a channel.
</Warning>

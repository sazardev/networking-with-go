...
<DeepDive title="cmd/ for multiple binaries">
The `cmd/<name>/main.go` pattern exists specifically for projects that produce more than one binary from the same module — an API server and a separate worker process, for instance, both importing shared code from `internal/`. For a single-binary service, `cmd/myapi/main.go` might feel like unnecessary nesting, but it costs nothing and keeps the door open if a second binary shows up later.
</DeepDive>

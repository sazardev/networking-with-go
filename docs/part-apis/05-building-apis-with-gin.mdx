...
<DeepDive title="gin.Default() vs gin.New()">`gin.Default()` attaches the `Logger()` and `Recovery()` middleware automatically — good defaults for development and most production services. `gin.New()` gives you a bare engine with no middleware at all, useful when you want full control over logging format or need to swap in your own recovery/observability stack (for example, wiring in OpenTelemetry tracing middleware instead).</DeepDive>

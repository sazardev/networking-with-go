...
<Warning title="An access token is not proof of identity">
OAuth2 on its own only proves the app was granted some scope of access — the userinfo endpoint call above is what tells you who the user is, and it's a convention some providers support, not part of the OAuth2 spec itself. If identity matters (and it usually does for login), reach for OpenID Connect instead of trying to bolt identity onto plain OAuth2 yourself.
</Warning>
...
<DeepDive title="sub, not email, is the identifier to store">
Email addresses can be reused or changed by the identity provider; the `sub` claim is a stable, provider-scoped identifier meant precisely for linking to your local user record. Store `sub` (and which provider it came from) as your foreign key, and treat `email` as a display value you refresh on each login.
</DeepDive>
...
<Warning title="A refresh token is more sensitive than the access token it renews">
An access token expires in minutes and its blast radius is limited by that short window; a refresh token can mint new access tokens indefinitely and is exactly what an attacker wants if your storage is compromised. Encrypt refresh tokens at rest and treat their storage with the same care you'd give a password hash, not the same casualness you might give a short-lived session token.
</Warning>

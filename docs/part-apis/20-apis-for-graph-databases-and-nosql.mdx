...
<Warning title="Flexible schema is not the same as no schema">Just because MongoDB doesn't enforce a schema at write time doesn't mean your API should skip validation. Decode into a typed struct, validate required fields explicitly, and consider MongoDB's own JSON Schema validation on the collection if multiple services write to it.</Warning>

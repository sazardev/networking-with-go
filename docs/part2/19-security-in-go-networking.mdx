...
**Why does an ephemeral ECDHE key exchange matter if the server already has a certificate?**
Because a certificate only proves who signed a public key — it doesn't protect past traffic if that long-term private key is ever stolen. The handshake's key-exchange keypair is generated fresh and discarded right after use, which is what gives TLS forward secrecy: an attacker who steals tomorrow's private key still can't decrypt a session recorded today.

...
<DeepDive title="The STRIDE categories">
**Spoofing** — pretending to be something or someone you're not (a fake server, a forged source IP, a stolen session token). **Tampering** — modifying data or code in transit or at rest without authorization. **Repudiation** — being able to deny having performed an action, because nothing logged it. **Information disclosure** — exposing data to someone not authorized to see it. **Denial of service** — degrading or blocking legitimate access to a service. **Elevation of privilege** — gaining capabilities beyond what was granted, such as an unauthenticated user reaching an admin-only route.
</DeepDive>
...
type EntryPoint struct {
	...
	Protocol       string
	RequiresAuth   bool
	InternalOnly   bool // only reachable from the internal network, not the public internet
...
func main() {
	...
	for _, e := range surface {
		fmt.Printf("%-14s proto=%-5s auth=%-5v internal=%-5v risk=%d\n",
			e.Name, e.Protocol, e.RequiresAuth, e.InternalOnly, riskScore(e))
	}
}
```

...
var checks = []headerCheck{
	{"Strict-Transport-Security", "forces browsers to use HTTPS on future visits", true},
	{"X-Content-Type-Options", "prevents MIME-type sniffing (expect \"nosniff\")", true},
	{"X-Frame-Options", "mitigates clickjacking via framing", false},
	{"Content-Security-Policy", "restricts which sources can execute/load content", false},
}

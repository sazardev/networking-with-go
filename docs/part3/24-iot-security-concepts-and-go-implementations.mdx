...
**What happens to a device if the vendor stops supporting it but it's still working fine?**
It becomes exactly the risk this chapter describes: a device that will eventually run known-vulnerable firmware forever, since there's no update mechanism left to close new vulnerabilities as they're discovered. The practical mitigations at that point shift from "patch it" to containment - tighter network segmentation, stricter egress allowlisting, and treating the device as permanently untrusted on the network even though it still does its job.

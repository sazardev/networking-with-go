...
<Warning title="Snapshot Before, Isolate Always">
Take a clean snapshot of every VM before running anything offensive against it, so a lab session can always be reset to a known-good state regardless of what happens during it. Double-check network isolation before every session that involves live malware, exploitation tooling, or anything you wouldn't want to accidentally reach a real network - a misconfigured bridge adapter connecting a "isolated" lab VM back to your home or corporate network is a genuinely common mistake, not a hypothetical one.
</Warning>

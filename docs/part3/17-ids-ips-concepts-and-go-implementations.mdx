...
<DeepDive title="Where IDS/IPS Sits in the Stack">Network-based IDS/IPS (NIDS/NIPS) inspects traffic at chokepoints like a gateway or span port and sees everything crossing that link, but nothing that stays local to a host. Host-based IDS/IPS (HIDS/HIPS) runs on the endpoint itself, watching system calls, file integrity, and local logs, and catches activity that never touches the wire. The two are complementary, not competing, layers of the same defense-in-depth strategy discussed earlier in the book.</DeepDive>
...
<Warning title="Tuning Before Trusting">A freshly deployed IPS in blocking mode, with default signatures and no tuning for your environment's actual traffic, is a common cause of self-inflicted outages. Standard practice is to run new rule sets in detection-only mode first, review the alert volume against real traffic for days or weeks, and only promote well-understood, low-noise rules to active blocking.</Warning>
...
**Can anomaly-based detection replace signature-based detection entirely?**
Not in practice, no. Signatures give you fast, precise, low-noise matches for attacks that are already known, while anomaly detection exists specifically to catch the zero-day or never-before-seen tool that has no signature yet. Most production stacks run both together, on the same engine, because each covers the other's blind spot.

**How does NIDS/HIDS placement connect to the segmentation ideas from the previous chapter?**
Segmentation controls which paths traffic is even allowed to take between zones; IDS/IPS decides what to do with the traffic that's already permitted to flow along those paths. A network sensor at a chokepoint between segments can watch everything crossing that boundary, while a host-based sensor picks up local activity - like a process spawning oddly - that never crosses a segment boundary at all, so it never reaches a network sensor to begin with.

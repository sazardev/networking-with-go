...
<DeepDive title="The typical phases of an engagement">**Scoping and rules of engagement** — agreeing in writing exactly what's in scope, what's off-limits, the testing window, and emergency contacts if something goes wrong. **Reconnaissance** — gathering information about the target passively (public records, DNS, job postings) and actively (scanning, per Chapter 4.4). **Vulnerability analysis** — identifying candidate weaknesses (Chapter 4.6) worth pursuing. **Exploitation** — attempting to actually leverage a weakness to gain access or demonstrate impact, carefully and within scope. **Post-exploitation** — assessing what the access is actually worth: what data, systems, or further access it provides, without exceeding the agreed boundaries. **Reporting** — the deliverable that actually matters to the client: findings, evidence, severity, and concrete remediation guidance.</DeepDive>

...
func main() {
	...
	server := &http.Server{
		...
		TLSConfig: &tls.Config{
			MinVersion: tls.VersionTLS12, // reject anything older than TLS 1.2
		},
	}
...
func main() {
	...
	conn, err := tls.Dial("tcp", "example.com:443", &tls.Config{
		...
		ServerName: "example.com", // used for SNI and certificate hostname verification
	})
	if err != nil {
...
<Warning title="Never ship InsecureSkipVerify: true">
Code with `tls.Config{InsecureSkipVerify: true}` will work perfectly in testing and then silently accept a forged certificate in production. If you need to trust a private or self-signed CA, add it to a custom `RootCAs` certificate pool (Chapter 4.13) instead of disabling verification altogether.
</Warning>
...
[Previous: Further Cybersecurity Resources](11-further-cybersecurity-resources.md) | [Next: Certificate Management and PKI](13-certificate-management-and-pki.md)

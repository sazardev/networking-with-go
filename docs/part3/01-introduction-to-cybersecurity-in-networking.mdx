...
- **Confidentiality:** Can someone on the path between two hosts read data they shouldn't? This is why we encrypt traffic with TLS (Chapter 5.12) instead of sending credentials in plaintext.
...
- **Assume breach:** design as if an attacker is already somewhere on your network, and ask what limits the damage they can do from there. This idea reappears fully in Chapter 5.15 (Zero Trust).
...
- **Process** (5.8–5.11): how penetration tests, incident response, and ethical/legal boundaries actually work in practice.
- **Cryptography and trust** (5.12–5.13): TLS and the PKI that makes it trustworthy.
- **Secure design** (5.14–5.16): writing safer Go code, and structuring networks around zero trust and segmentation.

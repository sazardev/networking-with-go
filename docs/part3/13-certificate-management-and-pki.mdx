...
Chapter 4.12 showed that TLS encrypts and authenticates a connection — but authenticates it *to whom*? Encryption alone doesn't prove you're talking to the right server; an attacker performing a man-in-the-middle attack (Chapter 4.3) can happily encrypt traffic too, just with themselves in the middle. A **certificate** is what closes that gap: a signed statement binding a public key to an identity (typically a domain name), issued by a party the client already trusts.

...
<DeepDive title="Why intermediates exist at all">
If every server certificate were signed directly by a root, the root's private key would need to be online and used constantly — a huge attack surface for the single most sensitive key in the whole system. Intermediates let the root stay offline almost all the time, only used rarely to sign new intermediates, while day-to-day issuance happens through keys that are easier to rotate or revoke if something goes wrong.
</DeepDive>
...
<Warning title="An expired certificate is a self-inflicted outage">
Unlike most security failures, a certificate silently expiring is entirely predictable and entirely preventable — track expiry dates for every certificate you operate, alert well before expiry, and prefer automated renewal over a calendar reminder.
</Warning>

---

[Previous: Implementing TLS/SSL in Go](12-implementing-tls-ssl-in-go.md) | [Next: Secure Coding Practices in Go](14-secure-coding-practices-in-go.md)
